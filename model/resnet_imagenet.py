...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if rp:
            self.rp_conv1 = Conv2d(3, self.inplanes - rp_out_channel, kernel_size=7, stride=2, padding=3, bias=False)
            self.rp1 = nn.Conv2d(3, rp_out_channel, kernel_size=7, stride=2, padding=3, bias=False)
        else:
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.rp:
            conv_x = self.rp_conv1(x)
            x = self.rp_forward(x, conv_x, self.rp1)
        else:
            x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
